...
const en = {
  launchSuccess: "Successfully launched",
  launchBadge: "Launched",
  launchNotStarted: "Not launched",
  launchPanelTitle: "Launch confirmation",
  launchManage: "Manage launch record",
  launchRevokedDate: "Revoked launch date",
  launchManageHelp: "Corrections and revocations require a reason. Previous records are retained.",
  launchDelayEnded: (date: string) => `Project launched on ${date}. Delay planning has ended.`,
  launchPending: "Awaiting launch confirmation",
  launchOverdue: "Launch confirmation overdue",
  launchConfirm: "Mark launch successful",
  launchCorrect: "Correct launch date",
  launchRevoke: "Revoke launch mark",
  launchRecord: "View launch record",
  launchMenu: (title: string) => `Launch actions for ${title}`,
  launchReady: "Acceptance passed. Confirm after the actual launch.",
  launchConfirmDescription: "Confirm that deployment is complete and working, then record the actual launch date.",
  launchCorrectDescription: "Correct the actual launch date and explain why. Previous records are preserved.",
  launchRevokeDescription: "Keep the history, restore the previous delay plan, and return to awaiting launch confirmation. Explain why you are revoking the mark.",
  launchSaving: "Saving…",
  launchSaveFailed: "The launch record was not saved. Your input is preserved; please retry.",
  launchRefreshFailed: "The launch record was saved. Refresh the view to see the latest state.",
  launchSaved: (title: string, date: string) => `${title} marked as successfully launched. Actual launch date: ${date}.`,
  launchCorrected: "Launch date corrected",
  launchRevoked: "Launch mark revoked",
  launchUnchanged: "The record is unchanged.",
  launchHistoryEmpty: "No launch records yet.",
  launchAcceptanceUnknown: "The historical acceptance date is unknown. Verify the date against your deployment records.",
  launchDateHelp: (date: string, today: string) => `Choose a date from ${date} through ${today}.`,
  launchAfterWork: (count: number) => `${count} open task(s) after launch. The launch record is preserved.`,
  launchDelayLocked: "Launch is recorded. Revoke the launch mark before changing delay plans.",
  launchStale: "Project settings changed elsewhere. Reopen delivery gates before saving.",
  launchFromBaseline: "Compared with baseline",
  launchFromForecast: "Compared with confirmed forecast",
  launchVariance: (days: number, weekends: boolean) => days === 0 ? "On time" : `${englishScheduleDays(Math.abs(days), weekends)} ${days > 0 ? "late" : "early"}`,
  launchBlockSchedule: "Complete the delivery gate schedule first.",
  launchBlockAcceptance: "Complete acceptance and its prerequisite gates before confirming launch.",
  launchBlockDraft: "Confirm or withdraw the current delay assessment first.",
  launchBlockAlready: "Launch is already recorded. Use Correct launch date to change it.",
  launchBlockMissing: "There is no active launch record to change.",
  launchBlockHistory: "The previous delay state cannot be recovered from this history. Restore the missing delay history before revoking launch.",
  launchViewAcceptance: "View acceptance blockers",
  launchViewDelay: "View delay assessment",
  ...
  gateSameDayRuleTitle: "Same-day gate capacity",
  gateSameDayRuleDesc: "When consecutive gates share a project-clock day, choose whether that day remains available for delivery work.",
  gateSameDayToggleLabel: "Same-day gates",
  gateSameDayEnabled: "Keep 1 planning day",
  gateSameDayDisabled: "Keep 0 days",
  gateSameDayPreviewEnabled: "1d",
  gateSameDayPreviewDisabled: "0d",
  ...
  gateDelayClockPendingImpact: "Saving recalculates stage windows, capacity, delay values, planned progress, remaining time, and overdue time.",
  ...
  gateLaunchRecord: "Mark launch successful",
  ...
  gateRiskBaselineTrack: "Original plan",
  gateRiskForecastTrack: "Delayed",
  gateRiskActualTrack: "Actual",
  ...
  todayCompletedCount: (count: number) => `${count} task${count === 1 ? "" : "s"}`,
  todayCompletedDateOnly: "Date only",
  noTodayCompleted: "No completed tasks recorded today.",
  todayCompletedMissingDates: (count: number) =>
    `${count} completed task${count === 1 ? "" : "s"} have no usable completion date, so we can't confirm whether they were completed today.`,
  todayCompletedNote: "Only tasks with a recorded completion date are counted.",
  ...
  memberHealthNormal: "On track",
  memberHealthAttention: "Needs attention",
  ...
  memberGateSetupBody: (count: number) =>
    `${count} related project${count === 1 ? " is" : "s are"} missing a complete gate schedule. Work without a resolved delivery stage cannot enter the runway, window progress, or planned load; the delivery ledger remains available.`,
  ...
  memberDriverGate: (count: number) => `${count} gate-blocking task${count === 1 ? "" : "s"}`,
  memberDriverUnestimated: (count: number) => `${count} unestimated task${count === 1 ? "" : "s"}`,
  memberDriverUnscheduled: (count: number) =>
    `${count} task${count === 1 ? " has" : "s have"} no resolved delivery stage`,
  memberDriverOverrun: (count: number) => `${count} task${count === 1 ? "" : "s"} over plan`,
  filterTasksByInsight: (label: string) => `Filter tasks by ${label}`,
  loadWindow: "Load window",
  setLoadWindow: (days: number) => `Use the next ${days} schedule days`,
  compactDays: (days: number) => `${days}d`,
  customWindow: "Until",
  customWindowEnd: "Custom load-window end date",
  includeWeekendsInCapacity: "Include weekends",
  deliveryRunway: "Personal delivery runway",
  deliveryRunwayWindow: (start: string, end: string, days: number, includeWeekends: boolean) =>
    `${start} → ${end} · ${englishScheduleDays(days, includeWeekends)}`,
  capacityGap: "Shortfall",
  capacityBuffer: "Buffer",
  today: "Today",
  noWindowCommitments: "No estimated work is due in this window",
  checkpointFilterLabel: (date: string, count: number) =>
    `${date} checkpoint · ${count} due`,
  runwayCheckpointAria: (
    date: string,
    count: number,
    remaining: number,
    available: number,
    state: string
  ) => `${date}: ${count} due; ${remaining}h committed of ${available}h available; ${state}`,
  capacityPair: (committed: number, available: number) =>
    `${committed.toLocaleString(undefined, { maximumFractionDigits: 2 })}h / ${available.toLocaleString(undefined, { maximumFractionDigits: 2 })}h`,
  runwayLegendCommitment: "Node: cumulative commitments",
  runwayLegendCapacity: "Rail: available capacity over time",
  runwayLegendNode: "Select a node to inspect its tasks",
  windowProgress: "Window progress",
  filterWindowTasks: "Show tasks in the current window",
  plannedClosureDetail: (closed: number, total: number) =>
    `${closed.toLocaleString(undefined, { maximumFractionDigits: 2 })}h of ${total.toLocaleString(undefined, { maximumFractionDigits: 2 })}h closed`,
  taskClosureDetail: (closed: number, total: number) => `${closed} of ${total} tasks closed`,
  plannedLoad: "Planned load",
  filterWindowLoad: "Show unfinished commitments in the current window",
  ...
  projectMixFilterLabel: (project: string) => `${project} work`,
...
const zh: typeof en = {
  launchSuccess: "已上线成功",
  launchBadge: "已上线",
  launchNotStarted: "未上线",
  launchPanelTitle: "上线确认",
  launchManage: "管理上线记录",
  launchRevokedDate: "已撤销日期",
  launchManageHelp: "更正或撤销需填写原因，历史记录会保留。",
  launchDelayEnded: (date: string) => `项目已于 ${date} 上线，延期计划已结束。`,
  launchPending: "待确认上线",
  launchOverdue: "上线确认逾期",
  launchConfirm: "标记上线成功",
  launchCorrect: "更正上线日期",
  launchRevoke: "撤销上线标记",
  launchRecord: "查看上线记录",
  launchMenu: (title: string) => `${title} 的上线操作`,
  launchReady: "验收已通过，请在实际上线后确认。",
  launchConfirmDescription: "确认你已完成实际部署并验证可用，记录本次实际上线日期。",
  launchCorrectDescription: "更正实际上线日期并说明原因，原记录会保留在交付历史中。",
  launchRevokeDescription: "撤销后保留历史记录，恢复待确认上线及此前的延期计划。请说明撤销原因。",
  launchSaving: "正在保存…",
  launchSaveFailed: "上线记录未保存，请重试。你填写的内容已保留。",
  launchRefreshFailed: "上线记录已保存，请刷新视图查看最新状态。",
  launchSaved: (title: string, date: string) => `${title} 已标记上线成功，实际上线日期 ${date}。`,
  launchCorrected: "上线日期已更正",
  launchRevoked: "上线标记已撤销",
  launchUnchanged: "记录没有变化。",
  launchHistoryEmpty: "暂无上线记录。",
  launchAcceptanceUnknown: "历史验收日期未知，请根据部署记录核对实际上线日期。",
  launchDateHelp: (date: string, today: string) => `可选择 ${date} 至 ${today} 之间的日期。`,
  launchAfterWork: (count: number) => `上线后有 ${count} 项待处理工作，上线记录保留。`,
  launchDelayLocked: "已记录上线。请先撤销上线标记，再调整延期计划。",
  launchStale: "项目设置已在其他位置更新，请重新打开交付门禁后再保存。",
  launchFromBaseline: "较原计划",
  launchFromForecast: "较确认预计",
  launchVariance: (days: number, weekends: boolean) => days === 0 ? "按时上线" : `${days > 0 ? "晚" : "提前"} ${chineseScheduleDays(Math.abs(days), weekends)}`,
  launchBlockSchedule: "请先完善交付门禁排期。",
  launchBlockAcceptance: "请先完成验收及其前置门禁，再确认上线。",
  launchBlockDraft: "请先确认或撤销当前延期评估。",
  launchBlockAlready: "已记录上线，请使用“更正上线日期”调整。",
  launchBlockMissing: "当前没有可调整的上线记录。",
  launchBlockHistory: "无法从历史中还原此前的延期状态，请先恢复缺失的延期历史，再撤销上线。",
  launchViewAcceptance: "查看验收阻塞",
  launchViewDelay: "查看延期评估",
  viewName: "PM 洞察",
  commandOpen: "打开工作量洞察",
  ...
  gateWeekendSunday: "日",
  gateSameDayRuleTitle: "同日门禁容量",
  gateSameDayRuleDesc: "连续门禁落在同一个项目时钟日时，可决定是否为当天保留交付工作容量。",
  gateSameDayToggleLabel: "同日门禁",
  gateSameDayEnabled: "保留 1 个计划日",
  gateSameDayDisabled: "保持 0 天",
  gateSameDayPreviewEnabled: "1天",
  gateSameDayPreviewDisabled: "0天",
  ...
  gateDelayClockPendingImpact: "保存后将重新计算阶段窗口、容量、延期天数、计划应达、剩余时间和逾期时间。",
  ...
  gateRiskBaselineTrack: "原计划",
  gateRiskForecastTrack: "延期",
  gateRiskActualTrack: "实际",
  ...
  todayCompleted: "今日完成",
  todayCompletedCount: (count: number) => `${count} 项任务`,
  todayCompletedDateOnly: "仅日期",
  noTodayCompleted: "今天还没有记录到完成任务。",
  todayCompletedMissingDates: (count: number) =>
    `${count} 项已完成任务缺少可用完成日期，无法确认是否今天完成。`,
  todayCompletedNote: "仅统计有完成日期的任务。",
  ...
  memberDriverGate: (count: number) => `${count} 个任务阻塞门禁`,
  memberDriverUnestimated: (count: number) => `${count} 个任务未估时`,
  memberDriverUnscheduled: (count: number) => `${count} 个任务的交付阶段未明确`,
  memberDriverOverrun: (count: number) => `${count} 个任务已经超支`,
  filterTasksByInsight: (label: string) => `按“${label}”筛选任务`,
  loadWindow: "负载窗口",
  setLoadWindow: (days: number) => `使用未来 ${days} 个计划日`,
  compactDays: (days: number) => `${days}天`,
  customWindow: "截止",
  customWindowEnd: "自定义负载窗口截止日期",
  includeWeekendsInCapacity: "包含周末",
  deliveryRunway: "个人交付跑道",
  deliveryRunwayWindow: (start: string, end: string, days: number, includeWeekends: boolean) =>
    `${start} → ${end} · ${chineseScheduleDays(days, includeWeekends)}`,
  capacityGap: "工时缺口",
  capacityBuffer: "工时余量",
  today: "今天",
  noWindowCommitments: "当前窗口没有已估时的到期工作",
  checkpointFilterLabel: (date: string, count: number) => `${date} 节点 · ${count} 项到期`,
  runwayCheckpointAria: (
    date: string,
    count: number,
    remaining: number,
    available: number,
    state: string
  ) => `${date}：${count} 项到期；累计承诺 ${remaining} 小时，可用 ${available} 小时；${state}`,
  capacityPair: (committed: number, available: number) =>
    `${committed.toLocaleString(undefined, { maximumFractionDigits: 2 })}h / ${available.toLocaleString(undefined, { maximumFractionDigits: 2 })}h`,
  runwayLegendCommitment: "节点：累计承诺工时",
  runwayLegendCapacity: "轨道：随时间增长的可用容量",
  runwayLegendNode: "点击节点查看相关任务",
  windowProgress: "窗口内进展",
  filterWindowTasks: "查看当前窗口内的任务",
  plannedClosureDetail: (closed: number, total: number) =>
    `已闭环 ${closed.toLocaleString(undefined, { maximumFractionDigits: 2 })}h / ${total.toLocaleString(undefined, { maximumFractionDigits: 2 })}h`,
  taskClosureDetail: (closed: number, total: number) => `已闭环 ${closed} / ${total} 个任务`,
  plannedLoad: "计划负载",
  filterWindowLoad: "查看当前窗口内未完成的承诺任务",
  remainingWorkContext: (remaining: number, later: number) =>
  ...
  teamOverall: "团队整体",
  projectMixFilterLabel: (project: string) => `${project}工作`,
  projectMixComparison: (value: number | null, team: number | null, hours: number) =>
