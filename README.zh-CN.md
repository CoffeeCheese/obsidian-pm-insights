...
交付进度读取所选项目的完整任务树，不受成员或任务抽屉筛选影响。设置页可以将一个或多个 Obsidian 层级标签映射为设计、开发和测试任务，例如 `type/dev` 也会匹配 `type/dev/frontend`。

| 进度 | 计算方式 |
| --- | --- |
| **设计 / 开发 / 测试** | 对应类型中已完成的叶子子任务 ÷ 全部有效叶子子任务。 |
| **验收** | 前置类型全部完成且主任务本身已完成的主任务 ÷ 全部有效主任务。进度条另以浅色区段标出待验收主任务。 |
| **总进度** | 各阶段进度乘以用户配置权重后汇总。权重合计必须为 100%。 |

- 每种类型都可以设置是否参与验收，以及无匹配任务时是否跳过统计。被跳过的阶段会从总进度权重中移除，其余权重按比例归一化。
- 同时命中多个类型、未分类、未关联主任务、缺少验收前置，以及主任务提前完成的记录会进入数据质量提示。
- 已取消主任务会排除整棵任务树；归档内容继续跟随面板现有设置。“统计父任务”开关不会改变交付进度规则。
