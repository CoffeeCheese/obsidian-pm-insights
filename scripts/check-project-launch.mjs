...
const evaluation = `(async () => {
  ...
  check("Management actions always visible above history",getLaunch().querySelector('.pmi-launch-management').tagName==="DIV"&&getLaunch().querySelector('.pmi-launch-management').nextElementSibling.classList.contains('pmi-launch-history'));
  ...
  check("Management keeps compact aligned buttons",managementButtons.length===2&&managementButtons.every(button=>button.getBoundingClientRect().height<=48)&&Math.abs(managementButtons[0].getBoundingClientRect().top-managementButtons[1].getBoundingClientRect().top)<2);
