...
- **团队与个人视角：** 从跨项目工作量进入成员任务、共享任务、个人仪表盘和个人交付账本。
...
| **成员洞察** | 每位成员承担了什么、当前窗口推进如何，个人交付风险与团队相比处于什么位置？ |
...
选择成员后，点击姓名旁的仪表按钮即可打开独立的右侧抽屉。成员详情页继续保持紧凑，个人进展、近期承诺、可用容量与交付质量则集中在一个视图中查看。

![使用中文虚构测试数据的个人仪表盘抽屉，展示交付健康度、可调负载窗口、交付跑道、窗口内进展、计划负载、团队对比、工作构成和个人交付账本](docs/assets/pm-insights-member-dashboard-focused.zh-CN.png)

*截图仅使用 README 虚构演示项目与成员。点击风险标签或跑道节点后，抽屉会关闭，并在背后的成员任务列表中筛出对应源任务。*

个人仪表盘由六组互相关联的信息组成：

| 视图 | 说明 |
| --- | --- |
| **交付健康度** | 提炼当前最需要介入的原因，例如逾期、容量缺口、门禁阻塞、未估时、期限未明确或工时超支。 |
| **个人交付跑道** | 在每个有效截止节点汇总累计剩余承诺工时，并与该节点前可用的个人容量对比。 |
| **窗口内进展** | 统计截至窗口末日应完成的有效任务和计划工时，包含尚未完成的逾期任务；已完成任务仅在有效截止时间不早于今天时进入窗口。 |
| **计划负载** | 截至窗口末日尚未完成的已估时承诺（含逾期承诺）÷ 个人可用容量。 |
| **团队对比** | 将个人的计划负载、计划工时完成率、逾期、超支、并行项目、共享工作和高优先级工作，与团队各项结果排序后的中间值对比；成员数为偶数时取中间两个值的平均。 |
| **工作构成与个人交付账本** | 按项目拆分个人窗口内剩余工时，并补充不受窗口限制的完成情况、工时执行和计划完整度。 |

负载窗口从今天开始，可以选择未来 7、14、30 个计划日，也可以自定义截止日期。手动切换**包含周末**后，计划日数量和每日容量规则会立即一起变化；工作日与自然日的每日工时取自 **设置 → PM Insights → 门禁风险规则**。

跑道和窗口统计会优先使用任务自身截止日期；没有任务截止日期时，依次使用所属阶段门禁和项目上线日期。共享任务的计划工时、已登记工时和剩余工时会在已识别的负责人之间平均分摊。如果相关项目尚未完整设置交付门禁，抽屉会说明：缺少任务截止时间的工作无法进入交付跑道、窗口内进展和计划负载，并提供直接的**设置门禁**入口；不受窗口限制的个人交付账本仍可查看。
...
个人仪表盘底部的交付账本会围绕三个通俗问题组织六项指标：任务有没有完成、实际投入是否符合计划、计划数据是否足以支撑判断。每项个人结果旁都会显示**团队参考**，即把团队有效结果从低到高排列后取中间值；成员数为偶数时取中间两个值的平均。

| 分组 | 指标 | 计算方式 |
| --- | --- | --- |
| **完成情况** | 任务完成率 | 已完成且未取消任务 ÷ 全部未取消任务。 |
| **完成情况** | 计划工时完成率 | 已完成任务的计划工时 ÷ 全部已估算工时。 |
| **工时执行** | 计划工时使用率 | 已估算任务的登记工时 ÷ 对应计划工时；超过 100% 代表实际登记已超过计划。 |
| **工时执行** | 工时超支任务 | 已开工且登记超过计划的任务 ÷ 全部已开工且有估算的任务。 |
| **计划完整度** | 估时准确率 | 登记工时处于估算 ±20% 范围内的已完成、已开工任务 ÷ 全部有效样本。 |
| **计划完整度** | 已估时任务占比 | 已填写计划工时的未取消任务 ÷ 全部未取消任务。 |
...
2. 选择一个或多个项目。
3. 选择成员，点击姓名旁的仪表按钮打开个人仪表盘；点击任一洞察可继续筛选对应源任务。
