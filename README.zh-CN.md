...
上线提醒会继续展示项目剩余总工作量，但不再把整个团队视为一个串行资源。系统会按别名归一后的负责人汇总阶段任务，共享任务的剩余工时在多位负责人之间平均分摊，并在每个有效阶段门禁和上线日期前累计同一人的工作；不同负责人可以并行，同一负责人跨阶段的工作仍会累加。每个节点以负载最高的负责人对比其个人可用容量，所有节点中最严重的结果决定容量提醒：出现缺口为高风险，利用率达到 80% 提示关注，未估时、未分配或未映射阶段的工作会作为计划盲点单独提示。用户可在 **设置 → PM Insights → 门禁风险规则** 分别设置“每人每个工作日可用工时”和“每人每个自然日可用工时”；已有配置保持兼容，默认均为每人 8 小时。
