...
- **团队与个人视角：** 从跨项目工作量进入成员任务、共享任务、项目级工作负载和交付容量窗口。
- **交付与验收视角：** 依据任务层级、阶段标签和任务状态推导阶段完成度与需求验收结果。
- **数据质量视角：** 识别未估时、未分配、未分类、类型冲突和缺少前置等问题。
- **计划与风险视角：** 对比阶段实际进度、计划应达比例、任务截止时间、项目门禁日期和负责人可用容量。
...
| **成员洞察** | 每位成员承担了什么、近期有哪些交付窗口，当前项目工作能否装入各交付日前的剩余工时容量？ |
| **交付进度** | 工作推进到了哪个阶段，距离验收还有多远？ |
| **交付异常** | 哪些任务没有被正确纳入阶段，或正在阻塞验收？ |
| **门禁风险** | 当前进度和负责人容量是否跟得上阶段门禁与最终上线计划？ |
...
选择成员后，点击姓名旁的仪表按钮即可打开居中的**个人交付简报**。这个弹窗聚焦一个关键判断：这个人当前承担的项目工作，能否在各项目交付日前装入剩余工时容量？顶部会把最紧窗口明确表达为**阶段起点 → 工作日数 → 交付节点**，余量或缺口结论则只在下方独立的交付容量卡片中呈现。

![使用中文虚构测试数据的个人交付简报，展示阶段起点到交付节点的清晰路线、交付窗口、项目工作负载和唯一的交付容量对比](docs/assets/pm-insights-member-dashboard-focused.zh-CN.png)

*截图仅使用“个人仪表盘演示”虚构项目与模拟成员。顶部只负责说明最紧阶段区间，不重复容量结论；下方交付容量卡片负责负载比对，其中刻度表示可用容量上限，斜纹段表示交付节点前无法装下的超载工时。*

个人交付简报将规划信息与今日完成活动栏放在一起：

| 视图 | 说明 |
| --- | --- |
| **交付窗口** | 只展示规划范围内由阶段推导出的交付日期。同一天的项目与阶段会合并为一个窗口，并展示任务进展、已闭环与剩余工时、累计负载与阶段窗口容量以及具体风险信号。 |
| **今日完成** | 按设备本地今天列出该成员有原生完成日期的任务。有具体时刻的记录按最近完成优先排列，每项展示任务及项目，并可在 Project Manager 中打开源任务。只有日期的记录会标为“仅日期”；只有已完成状态的任务无法归入某一天。 |
| **项目工作负载** | 汇总当前项目范围内分配给该成员的全部有效未完成工作，不受 7、14、30 天规划范围限制。每个项目展示剩余工时、个人分摊、任务数、交付阶段和推导出的交付日期。 |
| **交付容量** | 按推导出的项目交付日排序，在每个节点对比累计剩余工作与“上一阶段门禁→当前交付节点”之间的可用工时。较早项目会先占用容量，从而直接暴露余量、临界、缺口和已逾期状态。 |

点击交付窗口、项目工作负载行、容量节点或计划置信提示，会关闭弹窗并在成员任务列表中筛出对应源任务。规划范围从今天开始，只控制时间轴上展示哪些交付窗口：可以选择未来 7、14、30 个计划日，也可以自定义截止日期。项目工作负载和交付容量始终覆盖当前所选项目范围内的全部现有工作。切换**包含周末**只会改变计划日计数和容量计算，不会改变哪些任务进入工作负载；工作日与自然日的每日工时取自 **设置 → PM Insights → 门禁风险规则**。

成员工作会先按项目分组，再以该成员在项目中覆盖的最远交付阶段确定阶段门禁日期；该节点的容量从上一个已配置阶段门禁开始计算，而不是从个人仪表盘的“今天”或规划范围起点开始计算。父任务会继承其可执行后代覆盖的阶段；跨阶段工作按配置顺序中的最远阶段处理。任务自身截止日期只作为排期风险信号，不用于定位项目交付窗口，项目上线日期也不会作为兜底日期。共享任务的计划、已登记和剩余工时会在已识别的负责人之间平均分摊；未估时和无法解析阶段的工作会作为计划盲区明确展示，不会虚构日期或工时。
...
上线提醒会展示项目剩余总工时，但不会把整个团队当成一个串行资源。容量判断关注的是：**在每个阶段节点前，负载最高的负责人是否能完成自己名下的剩余工作。**

| 规则 | 处理方式 |
| --- | --- |
| **不同负责人并行** | 分别汇总每个人的剩余工时，以负载最高者作为当前节点的瓶颈，不把多人工作简单相加。 |
| **同一负责人累加** | 同一人在前序与当前阶段的未完成工作会累计到对应节点，避免忽略跨阶段连续占用。 |
| **共享任务拆分** | 一项任务有多位负责人时，剩余工时会在别名归一后的负责人之间平均分摊。 |
| **逐节点评估** | 每个有效阶段门禁和上线日期都会计算一次个人可用容量，最严重的节点决定最终提醒。 |

例如，开发阶段两位负责人分别剩余 `60h` 和 `50h`，团队剩余总工时是 `110h`，但并行推进时的瓶颈负载是 `60h`，不是 `110h`。如果开发门禁前还有 5 个自然日、每人每天可用 8 小时，则个人容量为 `40h`，瓶颈负责人存在 `20h` 工时缺口。

![聚焦展示 PM Insights 上线提醒中的负责人工期瓶颈，包括剩余总工时、瓶颈负责人、个人负载、工时缺口以及各节点容量](docs/assets/pm-insights-launch-capacity-focused.zh-CN.png)

*上线提醒将团队总量与个人瓶颈分开表达：截图中的 142h 还包含乔月的 32h 测试工作；顾舟和陆川的 60h、50h 并行计算，开发节点以顾舟的 60h 对比个人可用 40h。*

容量超过可用工时会标记为高风险，利用率达到 80% 会提示关注；未估时、未分配或未映射阶段的工作不会被静默忽略，而是作为计划盲点单独提示。用户可在 **设置 → PM Insights → 门禁风险规则** 分别设置“每人每个工作日可用工时”和“每人每个自然日可用工时”；已有配置保持兼容，默认均为每人 8 小时。

#### 确认项目已上线成功

已选项目名称旁固定显示 **未上线 / 待确认上线 / 已上线**，点击打开独立的上线确认面板。验收通过、实际部署并验证可用后，点击 **标记上线成功**，填写实际上线日期。门禁风险详情的上线节点也可打开此面板；延期计划只展示上线后的结束说明。**更正上线日期** 和 **撤销上线标记** 常驻于结果卡下方，历史时间轴独立折叠。

![项目卡片上的未上线与已上线状态入口](docs/assets/pm-insights-launch-status-focused.zh-CN.png)

*点击项目名称旁的状态，直接打开该项目的上线确认面板。*

- 所有项目都需要人工确认上线，包括没有延期历史的项目。验收完成后显示“待确认上线”，不会自动生成上线记录。
- 首次确认要求有效排期、验收及前置门禁通过，并且没有待处理的延期评估。实际上线日期可以从验收实际通过日回填到今天。
- 保存成功后显示一次上线印记动画、项目“已上线”徽标和实际日期，并按项目时钟比较原计划与已确认预计日期。减少动态效果设置下直接显示完成态。
- **更正上线日期** 和 **撤销上线标记** 均要求填写原因并保留历史。撤销会恢复登记前的延期状态；上线期间的延期计划只供查阅。
- 上线后任务重新打开，仍保留上线事实并提示待处理工作。任务状态、工时、项目归档及任务笔记不会被自动修改。

![上线成功面板展示实际日期、计划偏差、常驻更正与撤销按钮，以及展开的上线记录时间轴](docs/assets/pm-insights-launch-records-focused.zh-CN.png)

*截图使用虚构演示项目，展示已保存的上线结果、常驻的两项管理操作，以及确认、更正和撤销组成的历史时间轴。输入控件统一使用细边框与单道焦点提示，在 Pixel 主题下也不会叠加厚重边框。*

上线记录保存在 PM Insights 自身设置中，不会执行真实部署。升级前已经保存的合法上线日期继续有效；此前仅因验收完成而自动通过的上线提醒会显示“待确认上线”。旧历史无法确定撤销后的延期状态时，插件会说明缺失信息并保留当前记录。
...
如果 Project Manager 设置为在标签页中编辑任务，Insights 会遵循该偏好。编辑操作由 Project Manager 负责，PM Insights 本身不会写入任务笔记。
...
2. 选择一个或多个项目。
3. 选择成员，点击姓名旁的仪表按钮打开个人交付简报；点击交付窗口、项目工作负载或容量节点可继续筛选对应源任务。
