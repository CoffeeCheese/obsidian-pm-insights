...
| 视图 | 说明 |
| --- | --- |
| **交付窗口** | 只展示规划范围内由阶段推导出的交付日期。同一天的项目与阶段会合并为一个窗口，并展示任务进展、已闭环与剩余工时、累计负载与容量以及具体风险信号。 |
| **项目工作负载** | 汇总当前项目范围内分配给该成员的全部有效未完成工作，不受 7、14、30 天规划范围限制。每个项目展示剩余工时、个人分摊、任务数、交付阶段和推导出的交付日期。 |
| **交付容量** | 按推导出的项目交付日排序，在每个节点对比累计剩余工作与累计可用工时。较早项目会先占用容量，从而直接暴露余量、临界、缺口和已逾期状态。 |
...
成员工作会先按项目分组，再以该成员在项目中覆盖的最远交付阶段确定阶段门禁日期。父任务会继承其可执行后代覆盖的阶段；跨阶段工作按配置顺序中的最远阶段处理。任务自身截止日期只作为排期风险信号，不用于定位项目交付窗口，项目上线日期也不会作为兜底日期。共享任务的计划、已登记和剩余工时会在已识别的负责人之间平均分摊；未估时和无法解析阶段的工作会作为计划盲区明确展示，不会虚构日期或工时。
